...
type EmailCheckStatus = 'idle' | 'checking' | 'new_user' | 'already_yours'
...
export function TraineeRegistrationModal({
  ...
  // Only the "already yours" case surfaces info: rule `trainerId == auth.uid`
  // legitimately grants the trainer read access to their own trainees.
  // Anything else (not found, permission-denied for other-trainer/unassigned
  // users, trainer profiles) collapses to 'new_user' so no existence leaks;
  // Firebase Auth will still reject a duplicate email at create time.
  const checkEmail = useCallback(async (email: string) => {
    ...
    try {
      ...
      if (foundUser && foundUser.role === 'user' && foundUser.trainerId === user.uid) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (emailCheckStatus === 'already_yours') return
  ...
  const canSubmit = emailCheckStatus === 'new_user' && !isSubmitting
      ...
      case 'already_yours':
        return (
          <div className="flex items-center gap-2 text-status-error text-sm">
            ...
            <span>המתאמן הזה כבר משויך אליך</span>
          </div>
        )
      default:
        return null
    ...
    <div className="modal-backdrop !z-[60]" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-dark-border sticky top-0 bg-dark-surface z-10">
          <h2 className="text-xl font-bold text-text-primary">רישום מתאמן חדש</h2>
