...
interface AuthenticationFlow {
  step1: {
    title: "כניסה ל-GymIQ";
    subtitle: "הזינו את מספר הטלפון שלכם";
    action: "בדיקת מספר טלפון במערכת";
    
    // אם משתמש חדש
    ifNewUser: {
      title: "רישום ל-GymIQ";
      subtitle: "בואו ניצור לכם חשבון";
      fields: [
        "מספר טלפון (כבר מולא)",
        "שם פרטי",
        "שם משפחה"
      ];
      primaryAction: "יצירת חשבון";
    };
    
    // אם משתמש קיים
    ifExistingUser: {
      title: "ברוכים השובים!";
      subtitle: `שלום ${firstName}`;
      primaryAction: "כניסה למערכת";
    };
  };
}
```
...
const handlePhoneSubmit = async (phoneNumber: string) => {
  ...
  if (existingUser) {
    // Login existing user immediately
...
// 2. If new user - complete registration
const handleRegistration = async (data: RegistrationDto) => {
