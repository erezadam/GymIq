...
## מאמן AI — רצפת סטגנציה להמלצות משקל (16/07/2026)

| # | תרחיש | ציפייה |
|---|-------|--------|
| SF-1 | תרגיל עם דגל "מומלץ להגדיל משקל" דלוק → צור אימון AI חדש → פתח את כרטיס התרגיל | ✅ ההמלצה המספרית גדולה ממש מהמשקל של האימון האחרון (לא שווה, לא קטנה) |
| SF-2 | תרגיל בלי דגל (ביצועים משתנים) | ✅ ההמלצה חופשית בטווח 95%-105% מהאחרון — ירידה קלה מותרת |
| SF-3 | Cloud Logging אחרי יצירה | ✅ אם הייתה דריסה — נרשם warn 'LLM recommendation overridden'; אם דגל חסר לתרגיל עם היסטוריה — 'stagnation flag missing where expected'. שקט מוחלט = ה-LLM המליץ נכון מעצמו |
| SF-4 | **מגבלת localStorage:** אימון AI שנוצר לפני התיקון וממשיך להיות בשימוש | ⚠️ ההמלצה השמורה (`continueAIRecommendations`) תישאר שגויה עד יצירת אימון AI חדש — זו לא רגרסיה, זה נתון ישן |
| SF-5 | סיום אימון ומיד יצירת תוכנית AI (race של fire-and-forget) | ✅ במקרה קצה הדגל טרם נכתב → אין רצפה לאותו תרגיל + warn 'missing' — התנהגות כמו לפני התיקון, לא גרועה ממנה |


## ספריית פרומפטים — קבוצות עריכה + בורר מודל (23/07/2026)

| # | תרחיש | ציפייה |
|---|-------|--------|
| PL-1 | פתח כרטיס "מאמן AI" ב-`/admin/prompts` | ✅ שתי קבוצות מופרדות: "בשליטתך — נשמר ומשפיע" (פרומפט/מודל/טוקנים) ו"נעול בקוד — לא נערך כאן" (רשימת הבטחות לקריאה בלבד) |
| PL-2 | קרא את אזור עורך הפרומפט של המאמן | ✅ אזהרת ענבר: טווח המשקל 95%-105% שבטקסט אינו שולט ברצפת המשקל — הרצפה נאכפת בקוד |
| PL-3 | שדה המודל | ✅ dropdown קשיח בלבד (אין הקלדה חופשית); ברירת המחדל מסומנת "(ברירת מחדל)" |
| PL-4 | בחר מודל אחר מהרשימה → שמור → צור אימון AI | ✅ הבקשה רצה על המודל שנבחר (Cloud Logging: 'AI prompt override applied') |
| PL-5 | מסמך aiPrompts עם מודל שאינו ברשימה (למשל ערך legacy חופשי) | ✅ השרת לא קורס: המודל נזרק עם warn 'not in allow-list' והיצירה רצה על ברירת המחדל; במסך מוצגת אזהרה "לא ברשימה" |
| PL-6 | "ערוך במסך מלא" | ✅ overlay מסך מלא עם X ‏44x44 + ‏Escape; עריכה נשמרת גם בחזרה לכרטיס |

---

## אבטחה — סגירת פערי הרשאה (13/08/2026)

> בדיקות אוטומטיות (emulator): `npm run test:rules` (32), `npm run test:functions` (6).
> אדום-לפני-ירוק מול החוקים הישנים: `npm run test:rules:redcheck` (14 exploits אדומים).
> **קריטי לפריסה:** rules ו-functions נפרסים בנפרד מ-hosting; ל-functions נדרש dispatch ידני.

| # | תרחיש | תוצאה צפויה |
|---|-------|-------------|
| SEC-1 | קריאה ל-`sendWelcomeEmail` ללא התחברות | ✅ נדחית (`unauthenticated`), אין ייצור קישור ואין מייל |
| SEC-2 | מאמן A מבקש welcome email עבור מתאמן של מאמן B | ✅ נדחית (`permission-denied`) |
| SEC-3 | welcome email עם מייל "מוברח" בפרמטר | ✅ נשלח למייל **מהרשומה** בלבד, הפרמטר מתעלם |
| SEC-4 | מאמן יוצר מתאמן → welcome email נשלח | ✅ עובד (זרימת רישום לא נשברה) — אימות במכשיר/סביבה חיה |
| SEC-5 | ניסיון יצירת `users` עם `role:'admin'` מהקונסול | ✅ נדחה ע"י ה-rules |
| SEC-6 | משתמש מנסה לשנות `role`/`trainerId` של עצמו למאמן | ✅ נדחה; ניתוק (`trainerId:null`) מותר |
| SEC-7 | בעל אימון מנסה לשנות `userId` של המסמך למשתמש אחר | ✅ נדחה |
| SEC-8 | מאמן קורא/כותב היסטוריה של מתאמן **לא** מקושר | ✅ נדחה; מקושר — עובד |
| SEC-9 | העלאת תמונת מתאמן ע"י מאמן; עריכת פרופיל עצמית; העלאת exercise-set ע"י אדמין | ✅ כולם עובדים; כתיבה ע"י לא-מורשה/סוג-קובץ לא-תקין נחסמת |
| SEC-10 | רגרסיית אדמין: עדכון role, קריאת כל היסטוריה, impersonation | ✅ ממשיך לעבוד |

---

## Machine Lens (ML) — זיהוי מכשיר מצילום (17/09/2026)

> מפרט: `docs/features/machine-lens-spec.md`. אין שמירת צילום בשום מקום; מכסה 20/יום (`machineLensUsage/{uid}_{date}`).

| # | תרחיש | ציפייה |
|---|-------|--------|
| ML-1 | צלם מכשיר עם כיתוב ברור (למשל "Leg Press") מכפתור המצלמה בספריית התרגילים | ✅ ה-`labelText` מוצג בראש ה-sheet; עד 3 התאמות רלוונטיות עם score ו-reasons; לחיצה על התאמה מוסיפה את התרגיל לאימון כמו בחירה רגילה |
| ML-2 | צלם מכשיר בלי שום כיתוב | ✅ הזיהוי מתבסס על `machineType` בלבד; מוצגות התאמות סבירות ללא labelText, בלי שגיאה |
| ML-3 | צלם מכונת פולי (cable) | ✅ בין ההתאמות מופיע גם תרגיל עם ציוד `cable_machine` וגם תרגיל עם ציוד `machine`; לכל אחד `reasons` שמסביר מדוע הותאם |
| ML-4 | צלם עצם שאינו ציוד (כוס קפה) | ✅ `machineType='unknown'`, `confidence=0` ⇒ מוצג "לא נמצא תרגיל מתאים"; אין קריסה ואין סינון שקט |
| ML-5 | בצע 20 זיהויים באותו יום ואז נסה זיהוי 21 | ✅ מוצגת הודעת מכסה בעברית; לא מתבצעת קריאה למודל; למחרת המכסה מתאפסת |
| ML-6 | נתק רשת באמצע זיהוי (מצב טיסה אחרי הצילום) | ✅ הודעת שגיאה בעברית ב-sheet; כפתור הסגירה (X) עובד והמשתמש חוזר לספרייה |

---

## Admin Add Exercise (AD) — הוספת תרגיל מהאדמין (18/09/2026)

> מפרט: `docs/features/admin-add-exercise-spec.md`. מכסות: טיוטות 30/יום (`aiDraftUsage`), תמונות 20/יום (`aiImageUsage`) — fail-closed. מסלול הטרמינל `/exercise-add` ממשיך לרוץ במקביל.

| # | תרחיש | ציפייה |
|---|-------|--------|
| AD-1 | הזן שם תרגיל בעברית (למשל "לחיצת חזה בשיפוע") וצור טיוטה | ✅ נוצר `exerciseDrafts` עם `createdBy` של האדמין; `status='draft_ready'`; הטופס מתמלא בכל השדות מערכים חוקיים בלבד |
| AD-2 | הזן שם תרגיל באנגלית (למשל "Incline Bench Press") | ✅ אותה זרימה כמו AD-1; שם עברי ואנגלי מופקים; category/primaryMuscle/equipment תקינים |
| AD-3 | צור טיוטה לתרגיל שכבר קיים דומה לו במערכת | ✅ `similar` מאוכלס; התרגילים הדומים מוצגים לאדמין לפני שמירה (מניעת כפילות) |
| AD-4 | ה-LLM מחזיר פלט לא-תקין (שדה חסר / ערך מחוץ לרשימה החוקית) | ✅ `status='failed'` + `error`; הודעת כשל מוצגת לאדמין — אין סינון שקט ואין טופס חצי-מלא |
| AD-5 | יצירת תמונה שמסתיימת ב-WebP גדול מ-250KB | ✅ `status='failed'` + `error`; אין העלאה של קובץ חורג ל-`exercise-images/` |
| AD-6 | לחץ "ייצר מחדש" על תמונה קיימת | ✅ נוצרת תמונה חדשה שמחליפה את הקודמת בטיוטה; הקריאה נספרת במכסת `aiImageUsage` |
| AD-7 | אשר ושמור את התרגיל | ✅ התרגיל נוצר דרך הטופס הקיים (validation + removeUndefined); `markDraftSaved` נקרא ו-`status='saved'`; התרגיל מופיע בספרייה |
| AD-8 | מצה את המכסה (30 טיוטות או 20 תמונות ביום) ונסה שוב | ✅ הודעת מכסה בעברית; לא מתבצעת קריאה למודל; כשל בקריאת מסמך המכסה ⇒ דחייה (fail-closed) |
| AD-9 | הקם את אותו שם תרגיל בטרמינל (`/exercise-add`) ובאדמין | ✅ אותם category / primaryMuscle / equipment / reportType בשתי הרשומות (הבדלי ניסוח בהוראות מותרים) |
