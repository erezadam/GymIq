...
## מאמן AI — בורר ציוד: toggle "הכל" + הסרת פריט (16/08/2026)

> רקע: התקלה שדווחה — מברירת המחדל "הכל נבחר", לחיצה על פריט **צמצמה** אליו במקום להסיר אותו, וכפתור "הכל" לא ניקה. תוקן ב-`equipmentSelection.toggleEquipment`. **באג מובייל — אימות במכשיר אמיתי חובה, לא רק jsdom.**

| # | תרחיש | תוצאה צפויה |
|---|-------|-------------|
| EQ-1 | פתח מאמן AI → מקטע "ציוד זמין" (ברירת מחדל) | ✅ כל הפריטים מסומנים; "הכל" מסומן |
| EQ-2 | מ"הכל נבחר", לחיצה על פריט בודד (למשל "מכשירים") | ✅ הפריט **מוסר** בלבד; שאר הפריטים נשארים מסומנים (לא מצטמצם לפריט אחד) |
| EQ-3 | לחיצה נוספת על אותו פריט | ✅ הפריט חוזר להיבחר → הכל מסומן |
| EQ-4 | לחיצה על "הכל" כשהכל מסומן | ✅ **מנקה הכל** (אף פריט לא נבחר) |
| EQ-5 | לחיצה על "הכל" כשריק/חלקי | ✅ בוחר את כל הפריטים |
| EQ-6 | בחירה חלקית → "צור" | ✅ נשלח `equipmentFilter` עם הבחירה בפועל; "הכל נבחר" → לא נשלח שדה |
| EQ-7 | **מובייל (375px, iOS Safari):** touch על "הכל" ועל פריט | ✅ תגובת מגע תקינה, יעד 44px, RTL, אין צמצום שגוי |
