...
| תיקון | תיאור |
|-------|-------|
| **תג מומלץ — ספירה לפי קבוצת שרירים** | תג "מומלץ X/10" במסך בחירת תרגילים סופר סטים לפי תת-שריר (upper_chest, mid_chest) במקום לפי קבוצת שרירים (chest). תוקן: כל תרגילי אותה קבוצה מציגים אותו מספר סטים |
| **תג מומלץ — תאריך עתידי בשבוע אחר** | כשתכנון אימון לתאריך בשבוע עתידי, הנתונים השבועיים הוצגו של השבוע הנוכחי. תוקן: תאריך בשבוע אחר מציג 0/10 כי עוד לא התאמנתי באותו שבוע |

### 📝 שינויים

| קובץ | שינוי |
|------|-------|
| `src/domains/exercises/components/ExerciseLibrary.tsx` | החלפת `getWeeklyMuscleSets` (לפי primaryMuscle) ב-`getWeeklySetsByCategory` (לפי category). הוספת `isScheduledInDifferentWeek` לזיהוי תאריך בשבוע עתידי |
...
| שיפור | תיאור |
|-------|-------|
| **פיצול שבועי** | GPT בוחר פיצול חכם לפי מספר אימונים: PPL / Upper-Lower / Full Body |
| **סדר תרגילים מובנה** | עוגן compound → compound נוסף → נתמך מכשיר → בידוד → פיניש |
| **חוקי התאוששות** | בדיקת שרירים ראשיים + משניים למניעת חפיפה יום אחרי יום |
| **כיסוי תתי-שריר** | חזה עליון/אמצעי/תחתון, משיכה אנכית/אופקית, סקוואט/הינג' |
| **חוקי ציוד** | העדפת משקולות חופשיות לעוגן, מכשיר לתרגיל נתמך |
| **התאמת משקל לפי lastReps** | קצה עליון של טווח → חלק עליון של משקל, קצה תחתון → חלק תחתון |

### 📝 שינויים

| קובץ | שינוי |
|------|-------|
| `functions/src/ai-trainer/openaiClient.ts` | החלפת system prompt ב-`buildSystemPrompt()` בלבד |

### ✅ תאימות מלאה

- פורמט JSON ללא שינוי
- שמות שדות זהים (weight, repRange, sets, reasoning)
- כללי משקל ±5%/±10% נשמרו והורחבו
- אפס שינויי קוד מעבר לטקסט ה-prompt
...
| **Toast-ים אבחוניים נשארו בפרודקשן** | הוסרו כל ה-toasts האבחוניים (1/7-7/7) |

### 📝 שינויים

| קובץ | שינוי |
|------|-------|
| `WorkoutHistory.tsx` | אימוני AI completed מנותבים ל-nonBundledWorkouts (מוצגים ב"שבועיים אחרונים") |
| `useActiveWorkout.ts` | הסרת diagnostic toasts, ניקוי console.logs אבחוניים |

### 🔍 ניתוח שורש הבעיה

- **דיווח:** "אימון לא נשמר"
- **מצב אמיתי:** האימון נשמר ב-Firebase בהצלחה, אבל לא הוצג בממשק
- **סיבה:** לוגיקת סינון ב-WorkoutHistory.tsx הסתירה אימוני AI שהושלמו מכל הסקשנים:
  - Bundles עם כל האימונים completed → הוסתרו (שורה 649)
  - Single AI completed → סוננו החוצה (שורה 664)
  - סקשן "שבועיים אחרונים" → לא הכיל אימוני AI בכלל
- **תיקון:** אימוני AI completed מנותבים לסקשן הרגיל "שבועיים אחרונים"

### 📚 לקחים ועדכוני SKILLs

| מסמך | עדכון |
|------|-------|
| `development-flow-SKILL.md` | הוספת כלל "בדוק תצוגה קודם" - כשמדווחים "נתונים לא נשמרים", לבדוק query/filter לפני מנגנון שמירה |
| `qa-testing-SKILL.md` | הוספת רגרסיה 30/01 + בדיקת תצוגת אימון AI בזרימת בדיקה מלאה |
...
| באג | תיקון |
|-----|-------|
| תצוגת סטים בהיסטוריה | פירוט כל סט בנפרד עם תמיכה בגרביטון/גומיות |
| Race condition ב-PersonalRecords | מניעת בלבול נתונים בלחיצות מהירות |
| תרגילי bodyweight ב-PR | השוואה לפי חזרות במקום משקל |
| isOverallBest לא עבד ל-bodyweight | סימון שיא לפי חזרות מקסימליות |
| תצוגת PR לתרגילי bodyweight | מציג "X חזרות" + "משקל גוף" במקום "0 ק"ג" |

### 📝 שינויים

| קובץ | שינוי |
|------|-------|
| `WorkoutCard.tsx` | פירוט כל סט + תגיות גרביטון/גומיות |
| `PersonalRecords.tsx` | תצוגה מותאמת לתרגילי bodyweight |
| `workoutHistory.ts` | לוגיקה נפרדת ל-bodyweight ב-getPersonalRecords ו-getExerciseHistory |

### ✨ שיפורים

- תרגילי משקל גוף (עליות מתח, מקבילים וכו') מושווים לפי **חזרות**
- תצוגת PR מותאמת: "12 חזרות - משקל גוף" במקום "0 ק"ג"
- `hasImproved` עובד נכון לתרגילי bodyweight (שיפור = יותר חזרות)

---

## [v1.11.0] - 2026-01-26

### 🚀 תכונה גדולה: תמיכה בתרגילי עזרה (גרביטון וגומיות)

| פיצ'ר | תיאור |
|-------|-------|
| **גרביטון** | תמיכה בתרגילים עם משקל נגד (Graviton/Assisted) |
| **גומיות** | תמיכה בתרגילים עם גומיות התנגדות |
| **בחירה גמישה** | אותו תרגיל יכול לתמוך בשניהם - המשתמש בוחר בזמן אימון |
| **ניהול גומיות** | מסך אדמין לניהול סוגי גומיות (שם, צבע, רמת התנגדות) |

### 📝 שלבים שהושלמו

| שלב | תיאור | סטטוס |
|-----|-------|-------|
| **שלב 1** | עדכון Types - הוספת שדות assistance ל-Exercise, Workout, Sets | ✅ |
| **שלב 2** | מסך ניהול גומיות באדמין (`BandTypeManager.tsx`) | ✅ |
| **שלב 3** | טופס תרגילים - checkboxes לבחירת assistanceTypes | ✅ |
| **שלב 4** | תצוגת תרגילים - אייקונים וסימונים ברשימות | ✅ |
| **שלב 5** | אימון פעיל - בחירת סוג עזרה ודיווח נתונים | ✅ |

### 📁 קבצים חדשים

```
src/domains/admin/components/BandTypeManager.tsx  # מסך ניהול גומיות
src/domains/exercises/types/bands.ts              # Types לגומיות
src/lib/firebase/bandTypes.ts                     # Firebase API לגומיות
```
...
| קובץ | שינוי |
|------|-------|
| `exercise.types.ts` | הוספת שדות `assistanceTypes`, `availableBands` |
| `active-workout.types.ts` | הוספת שדות `assistanceType`, `assistanceBand` |
| `workout.types.ts` | הוספת שדות assistance לסטים |
| `ExerciseCard.tsx` | UI לבחירת סוג עזרה (גרביטון/גומיות/ברירת מחדל) |
| `SetReportRow.tsx` | שדות דיווח דינמיים לפי סוג העזרה |
| `useActiveWorkout.ts` | לוגיקת שמירה/טעינה של נתוני assistance |
| `workoutHistory.ts` | שמירת נתוני assistance ל-Firebase |
| `WorkoutCard.tsx` | תצוגת נתוני assistance בהיסטוריה |
| `AdminLayout.tsx` | הוספת לינק לניהול גומיות |

### 🐛 באגים שתוקנו

| באג | תיקון |
|-----|-------|
| Race condition בסיום אימון | ביטול auto-save לפני שמירת סטטוס completed |
| Firebase undefined error | שימוש ב-conditional spreading לשדות אופציונליים |
| שמות גומיות לא מופיעים בעברית | טעינת bandNameMap מ-Firebase |
| Multi-select גומיות | שינוי ל-single-select (גומייה אחת לסט) |

### ✨ חוויית משתמש

- כפתורי בחירה נוחים (ברירת מחדל / גרביטון / גומיות)
- תצוגה ברורה של סוג העזרה הנבחר
- שמירת ושחזור העדפות בהמשך אימון
- תצוגת נתוני עזרה בהיסטוריית אימונים

### 🔧 טכני

- Firestore collection חדש: `bandTypes`
- שמירת `assistanceWeight` לגרביטון (מספר בק"ג)
- שמירת `assistanceBand` לגומיות (ID של גומייה)
- תמיכה בתרגילים שלא מגדירים סוגי עזרה (ברירת מחדל)

---

## [v1.10.106] - 2026-01-24

### 🐛 תיקון: המשך אימון לא מדווח יוצר אימון חדש

| בעיה | פתרון |
|------|-------|
| **תיאור** | כשממשיכים אימון לא מדווח מ-WorkoutHistory, נוצר אימון חדש במקום לעדכן הקיים |
| **סיבה** | Race condition ב-`initWorkout` שמחק את `gymiq_firebase_workout_id` בריצה חוזרת |
| **תיקון** | במקום למחוק את ה-ID, הקוד עכשיו בודק אם הוא קיים ומשתמש בו |

### 📝 שינויים בקוד

| קובץ | שינוי |
|------|-------|
| `useActiveWorkout.ts` | תיקון לוגיקת שמירת Firebase workout ID בשורות 262-270 |

---

## [v1.10.105] - 2026-01-24

### 🚀 תכונה: תג "חודש אחרון" בספריית תרגילים

| פיצ'ר | תיאור |
|-------|-------|
| **תג חדש** | תרגילים שבוצעו ב-30 יום האחרונים מקבלים תג "חודש אחרון" |
| **צבע סגול** | התג החדש בסגול להבדיל מתג "אחרון" הכתום |
| **לוגיקת עדיפות** | אם תרגיל בוצע גם באימון האחרון - מוצג רק "אחרון" |

### 📝 שינויים בקוד

| קובץ | שינוי |
|------|-------|
| `workoutHistory.ts` | הוספת פונקציה `getLastMonthExerciseIds()` |
| `index.css` | הוספת סגנון `.badge-last-month` בצבע סגול |
| `ExerciseLibrary.tsx` | טעינת נתוני חודש אחרון + לוגיקת תצוגה |

### ✨ חוויית משתמש

- משתמשים רואים באילו תרגילים השתמשו לאחרונה
- עוזר לגיוון אימונים ומעקב אחר שימוש בתרגילים
- טעינה במקביל לביצועים מיטביים (Promise.all)

---

## [v1.10.97] - 2026-01-23

### 🚀 תכונה: הסבר AI אחרי יצירת אימון

| פיצ'ר | תיאור |
|-------|-------|
| **Popup הסבר** | אחרי לחיצה על "צור אימון" מוצג popup עם הסבר מה-AI |
| **הנמקת בחירות** | Claude מסביר למה בחר את התרגילים ומה ההיגיון מאחורי האימון |
| **קבוצות שרירים** | הצגת תגיות שרירים בפופאפ לזיהוי מהיר |

### 📝 שינויים בקוד

| קובץ | שינוי |
|------|-------|
| `promptBuilder.ts` | הוספת בקשה להסבר בפורמט JSON |
| `types.ts` (Functions) | הוספת שדה `explanation` ל-ClaudeWorkoutResponse |
| `types.ts` (Functions) | הוספת שדה `aiExplanation` ל-GeneratedWorkout |
| `generateWorkout.ts` | העברת ההסבר מ-Claude לתגובה |
| `aiTrainer.types.ts` | הוספת שדה `aiExplanation` ל-AIGeneratedWorkout |
| `AITrainerModal.tsx` | הוספת popup הסבר אחרי יצירת אימון |

### ✨ חוויית משתמש

- המשתמש רואה את ערך ה-AI מיד אחרי היצירה
- הסבר בעברית ב-2-3 משפטים
- אם נעשה שימוש ב-Fallback (Claude לא זמין) - לא מוצג popup

---

## [v1.10.96] - 2026-01-22

### 🚀 תכונה: אינטגרציית Claude API למאמן AI

| פיצ'ר | תיאור |
|-------|-------|
| **Cloud Function** | יצירת אימונים דרך Claude API בשרת |
| **Rate Limiting** | הגבלה של 5 יצירות ליום למשתמש |
| **Fallback אוטומטי** | אם Claude נכשל - עובר אוטומטית ללוגיקה מקומית |
...
└── src/
    ├── index.ts
    └── ai-trainer/
        ├── generateWorkout.ts    # Cloud Function
        ├── claudeClient.ts       # Claude API wrapper
        ├── promptBuilder.ts      # בניית prompts
        ├── rateLimiter.ts        # הגבלה יומית
...
| קובץ | שינוי |
|------|-------|
| `aiTrainerService.ts` | קריאה ל-Cloud Function + fallback מקומי |
| `firebase.json` | הוספת הגדרות functions |
| `firestore.rules` | הוספת קולקציית aiTrainerUsage |

### 🔐 אבטחה

- API Key נשמר כ-Firebase Secret (לא בקוד)
- רק משתמשים מאומתים יכולים לקרוא ל-Function
- בדיקת userId תואם למשתמש המחובר

### 📋 פריסה נדרשת

```bash
# 1. הגדרת סוד Claude API
...
| פיצ'ר | תיאור |
|-------|-------|
| **הרחבת אימון** | כעת ניתן לפתוח כל אימון במקבץ לצפייה בתרגילים |
| **כמות תרגילים** | תוקן באג שגרם לחלק מהאימונים להיווצר עם פחות תרגילים |

### 📝 פרטי המימוש

#### AIBundleCard.tsx
- הוספת יכולת הרחבה לכל אימון במקבץ
- טעינת פרטי תרגילים מ-Firebase בלחיצה
- תצוגת רשימת תרגילים עם תמונות וכמות סטים

#### aiTrainerService.ts
- תיקון לוגיקת מילוי תרגילים חסרים
- אם אין מספיק תרגילים לשרירים הנבחרים, ממלא משרירים אחרים
- מבטיח תמיד את כמות התרגילים המינימלית לפי משך האימון
