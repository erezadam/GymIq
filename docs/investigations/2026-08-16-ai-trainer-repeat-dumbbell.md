...
- **וריאציה + חד-גוניות ציוד — השורש המשותף הוא סדר הבריכה:** רנדומיזציה של סדר הבריכה per-call (model-agnostic) עדיפה על temperature (ששובר gpt-5). המשתמש מכריע.
- **כלל עקביות-הציוד:** לשקול ריכוך/גיוון-חובה **ב-systemPrompt שב-Firestore** (כי הוא המקור החי), לא רק בקוד.
- **בורר הציוד:** באג נפרד (לא הפעיל בתקרית) — היפוך `toggleEquipment` + היפוך הטסטים שנועלים אותו.
- **ממשל פרומפט:** להכריע בין חלופה 1/2 (נספח א׳) כדי שהפרומפט לא יחיה מחוץ ל-git בלי גרסאות.
