...
| 3 | "התמונה לא נוצרה" באדמין | ⚠️ אובחן + תוקן ב-**PR-H #188 — ממתין לבדיקת ארז בערוץ** (הפונקציה האידמפוטנטית כבר פרוסה; הוכח בפרודקשן: קריאה כפולה ⇒ יצירה אחת ו-already-exists) | ערוץ image-gate; probe 18/09 |
...
| 9 | thumbnail של תמונות 2:1 חותך את מרכז התמונה (החץ) | ✅ נסגר — object-left לפאנל הסיום ב-thumbnail; אושר בעין בערוץ ומוזג (#186, 18/09) | PR #186 |
...
## עדכון 18/09 (ערב)
- **Cable Donkey Kickback שוחזר** לערכי הגיבוי (`category=gluteus_maximus, primaryMuscle=longissimus`) באישור ארז; `check:muscles` — All valid.
- טיוטת הבדיקה `exerciseDrafts/3Y9V1R468UKWWdnsLE2i` נמחקה (לא נוצר ממנה תרגיל).
- פארקינג: שדה `id` מספרי פנימי בחלק ממסמכי exercises דורס doc id בגיבויים בתבנית `{id, ...data}` — לשמור doc id בשדה נפרד.
