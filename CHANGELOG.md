...
- **ממתין:** אימות runtime — ניסיון ראשון (10:34 UTC) לא מצא שום אירוע יצירה ב-55 דק' על revision `00041-faf` → היצירה לא הגיעה ל-handler (חסימת לקוח/סביבה אחרת). **לא תנאי גלגול, לא גוללנו.** PR-1 פרוס אך לא אומת ב-runtime — ממתין ליצירה שמגיעה לפונקציה בחשבון בדיקה עם מכסה פנויה.
- **hosting:** מיזוג PR #172 לא הביא שינוי לקוח (`src/`/`public/`/`index.html` = ריק) — רק bump version/sw.
- PR-2 (בורר ציוד, #173) **לא מוזג** — באג מובייל, ממתין לאימות מכשיר לפני merge (auto-deploy).
