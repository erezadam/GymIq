...
❌ **Tests document product intent, not code behavior** - טסט מתעד את מה שהמוצר *צריך*, לא את מה שהקוד *עושה*. אם טסט "נועל" באג קיים (מאשר התנהגות שגויה), הוא חלק מהבאג — כשמתקנים, הופכים את הטסט לחוזה המוגן של ההתנהגות הנכונה, ומציינים בהודעת ה-commit שהטסט תיעד באג. (נוסף 16/08/2026 בעקבות `aiTrainerEquipmentSelector.spec` שנעל את באג בורר הציוד.)
❌ **Any list streamed to an LLM is order-biased input** - כל רשימה שמוזרמת למודל שפה (תרגילים, פריטים, מועמדים לבחירה) נחשבת **קלט מוטה-סדר** עד שהוכח אחרת. מודלים עוגנים על פריטים מוקדמים ברשימה. לפני הזרמת רשימה לפרומפט של מודל שבוחר ממנה — לערבב בנקודת האריזה (בשרת), או להוכיח שהסדר לא מטה. (נוסף 16/08/2026 בעקבות תקרית הטיית-המיקום במאמן ה-AI; ראה `functions/src/ai-trainer/poolShuffle.ts` ו-`docs/investigations/2026-08-16-ai-trainer-repeat-dumbbell.md`.)
❌ **No documentation claims without diff** - אסור לסוכן להצהיר שעדכן מסמך (CLAUDE.md, CHANGELOG.md, architecture.md, qa_scenarios.md, או כל קובץ doc אחר) בלי להציג למשתמש git diff מפורש או view מלא של החלק שעודכן לפני commit. חל על **שני הצדדים**: הסוכן לא מצהיר על עדכון בלי דיף, והמשתמש/ארכיטקט לא מאשים בחוסר עדכון בלי לראות grep מלא של main המסונכרן.
...
### ❌ חוק ברזל: אין `undefined` בכתיבות ל-Firestore

> **רקע:** ב-30/04/2026 נוסף שדה אופציונלי `videoWebpUrl?: string` ל-Exercise (PR #108). הטופס שלח `undefined` כשהשדה ריק. Firestore SDK דוחה כל שדה עם ערך `undefined` ב-`updateDoc()`/`setDoc()`/`addDoc()` — וכל עדכון של תרגיל קרס בפרודקשן עד שהבאג זוהה.

**כל קריאה ל-`updateDoc()`, `setDoc()` או `addDoc()` עם `Partial<T>` או `T` שמכיל שדות אופציונליים — חובה לעטוף את ה-payload ב-`removeUndefined()` לפני הקריאה.**
...
// ❌ לא נכון — אם יש שדה אופציונלי ב-data שערכו undefined, הקריאה תקרוס
await updateDoc(docRef, { ...data, updatedAt: serverTimestamp() })
```

**Helper זמין**: `src/lib/firebase/firestoreUtils.ts` מייצא את `removeUndefined<T>(obj)` ו-`isTimestamp(value)`. ה-helper שומר על Date, Firestore Timestamp, מערכים, ואובייקטים מקוננים — רק מסיר מפתחות עם `undefined`.

**גם בצד הטופס**: עדיף לא לשלוח `undefined` מלכתחילה (defense-in-depth). תיקון בצד service בלבד מסתיר את הכוונה ועלול להישבר אם בעתיד מישהו יעקוף את ה-helper.

**בדיקות חובה**: בדיקה התנהגותית עם mock של `updateDoc` (לא source-grep). דוגמה ב-`tests/firestore-undefined.spec.ts`.
...
- **אם אין preview channel זמין**, ההרצה הלוקאלית עם גישה ממכשיר על אותה רשת:
  ...
  ```
- **merge ל-main = פריסה אוטומטית מיידית לפרודקשן** (auto-deploy פעיל מאז 28/04/2026 דרך `on: push` ב-`.github/workflows/deploy.yml`). לכן אימות מכשיר חייב לקרות לפני merge.
...
## 🧭 קונבנציות עבודה (נוספו 17/09/2026)

> מתעד רק דפוסים שכבר הוכחו בפועל בעבודה השוטפת.

### א. ערוץ תצוגה מקדימה לפני מיזוג UI
כל שינוי UI חייב לעבור ערוץ תצוגה מקדימה (`firebase hosting:channel:deploy`) **לפני** מיזוג ל-main. אימות במכשיר נעשה על ה-preview, לא בפרודקשן.

### ב. פורמט סיום דוח סוכן — "סיכום לארכיטקט"
כל דוח סוכן מסתיים בבלוק קבוע:
- **בוצע:** מה הושלם בפועל
- **נכשל / נעצר:** מה לא הושלם ולמה
- **דורש הכרעה של ארז:** החלטות פתוחות
- **מה להעתיק לקלוד:** מה שצריך לעבור לתיעוד/לשיחה הבאה

### ג. הוספת תרגילים — רק דרך הסקיל `/exercise-add`
...
- תוצר סופי: קובץ WebP יחיד ≤250KB בפריסת RTL (מצב ההתחלה מימין).
- קובצי ביניים: PNG ללא איבוד.
- קובצי הסגנון ב-`assets/image-style` הם קנון — אסור לערוך אותם.
...
| 16/06/2026 | App Audit (אבחון ע"י המשתמש) אומת מחדש ע"י 6 סוכני אימות מקבילים. נמצא Critical: יצירת משתמש במסך ניהול המשתמשים (`UsersList.handleCreateUser`) קראה ל-`createUserWithEmailAndPassword` על מופע ה-auth **הראשי** → חיברה אוטומטית את המשתמש החדש (לרוב `role: user`) והעיפה את האדמין מהסשן. (PR-A מתוך תכנית רב-שלבית; שאר הממצאים — exitWorkout fallback, deps ב-useActiveWorkout, error-states, race-conditions — מתוכננים כ-PRs נפרדים.) | תוקן ב-PR #132: helper חדש `src/lib/firebase/createAuthUser.ts` → `createIsolatedAuthUser` יוצר את משתמש ה-Auth על **secondary app מבודד**, מוחק אותו ב-`finally` (try/catch פנימי שלא מסווה את השגיאה המקורית), ומחזיר `uid` בלבד — בידוד Auth בלבד. `handleCreateUser` קורא ל-helper; בחירת ה-role וכתיבת ה-Firestore נשארו ב-call-site עם `removeUndefined()`. שם ה-app כולל מונה מונוטוני בנוסף ל-`Date.now()` למניעת התנגשות. שירות יצירת המתאמן (`traineeAccountService`, אותו דפוס) **לא נגע** — איחוד למקור-אמת יחיד = PR עתידי. 6 בדיקות התנהגותיות חדשות ב-`tests/createAuthUser.spec.ts` (mock SDK, assert שהיצירה על ה-secondary auth + teardown ב-finally). אומת ידנית בדפדפן: האדמין נשאר מחובר אחרי יצירת משתמש. |
| 19/06/2026 | המעבר ממסך התרגילים לאימון הפעיל היה איטי — `handleStartWorkout` המתין ל-`await maybeCreateSelfStandaloneProgram()` (כתיבת Firestore) לפני `navigate('/workout/session')`. | תוקן ב-PR #139: הוסר ה-`await` (רץ כעת `void` fire-and-forget) כך שהניווט לא נחסם. תיקון race נלווה (נחשף ע"י `/code-review`): כשיצירת ה-program הפכה fire-and-forget, `setSelfStandaloneProgram` כותב ל-store **אחרי** שה-render-closure של `useActiveWorkout` נלכד. כל נקודות השמירה — initial save ב-`initWorkout` + 2 נתיבי `saveWorkoutHistory` ב-`doFinish` — קוראות כעת `programId/programSource/programDayLabel` טריים מ-`getState()` במקום מה-closure (שהוסר מה-destructure ב-`useActiveWorkout.ts:261`), כדי שקישור ה-`self_standalone` לא ייפול ב-finish מהיר והמאמן יראה את האימון ב"אימונים בודדים". נתיב ה-autosave כבר קרא טרי. בדיקה התנהגותית חדשה ב-`tests/firebaseIdRecovery.spec.ts` אומתה **RED ללא התיקון, GREEN איתו**. |
| 19/06/2026 | פיצ'ר: מעבר מאימון פעיל → ניתוח שבועי → הוספת תרגיל לאימון הפעיל (זרימת מתאמן). | נוסף ב-PR #140: כפתור "📊 מה חסר?" בפוטר `ActiveWorkoutScreen` → `navigate('/analysis')` (האימון לא נסגר). ה-`+` ב-`TrainingAnalysis` מזהה אימון פעיל (`ACTIVE_WORKOUT_STORAGE_KEY`) ומוסיף `addToWorkout=true` דרך helper `buildAddExerciseHref` → ניתוב לזרימת ה-merge הקיימת (`resumingFromLibrary`) במקום מחיקת האימון. **`/code-review` (high) תפס באג קריטי**: כפתור "חזרה לאימון" עם `navigate('/workout/session')` רגיל היה נופל ל-`!isTabCloseRecovery` ב-init gate → מחיקת `firebaseIdKey` → `addDoc` → מסמך `in_progress` כפול (חוק-הברזל firebaseWorkoutId, 10/05/2026). **תוקן** ע"י העברת `state:{resumingFromLibrary:true}` (אין תרגילים חדשים → merge ריק). בדיקת קומפוננטה חדשה `tests/trainingAnalysisReturnButton.spec.tsx` אומתה **RED ללא הדגל**. רק זרימת המתאמן העצמאי. |
| 23/06/2026 | פיצ'ר (PR1 מתוך תכנית רב-שלבית): שער כניסה בצד הלקוח למאמן ה-AI. המאמן ייבנה אך ורק מתרגילים שהמתאמן ביצע בפועל; הוחלט ששער כניסה חוסם מתאמן ללא מספיק היסטוריה (במקום fallback), והשער נשען על מספר התרגילים ה**שונים** (distinct) שבוצעו — לא על מספר האימונים. | נוסף ב-PR #141: פונקציית read חדשה `getDistinctPerformedExerciseIds(userId)` ב-`src/lib/firebase/workoutHistory.ts` — סורקת את **כל** ההיסטוריה (ללא `limit(10)`, בניגוד לאחות `getRecentlyDoneExerciseIds`), מדלגת על soft-deleted, ומשתמשת באותו predicate קיים `ex.isCompleted && ex.exerciseId`. זורקת בכשל קריאה כדי שהקורא יחסום. `AITrainerModal` גוזר את הספירה פעם אחת בפתיחה (טעינת הנתונים היחידה במודאל) דרך `useEffect` עם מצב טעינה קצר; הספירה נגזרת מ-`distinctExerciseCount===null` כדי למנוע הבהוב חד-פריימי של הטופס לפני השער (**ממצא CONFIRMED מ-`/code-review` high, תוקן**). מתחת לסף → מצב חסימה נייטרלי ומעודד (לא הבאנר האדום) + זרימת היצירה מוחלפת (אי אפשר לייצר). בסף ומעלה → הזרימה, ה-payload, והקריאה ל-`generateAIWorkouts` ללא שינוי. כשל/אין משתמש/אין היסטוריה → נחשב מתחת לסף (לא קורס). `MIN_DISTINCT_EXERCISES = 10` — מקור אמת יחיד. 7 בדיקות התנהגותיות חדשות (`tests/distinctPerformedExercises.spec.ts`, mock ל-`getDocs` + assert על ה-Set — לא source-grep). **לא נגעתי**: Cloud Function, fallback, `useActiveWorkout.ts`, `getPersonalRecords`. אין כתיבות Firestore. PR2 (סינון בריכת התרגילים בפועל) ו-PR3 (הודעת ההסבר) — נפרדים, אחרי PR1. |
| 24/06/2026 | פיצ'ר (PR2 מתוך 3): צמצום בפועל של בריכת התרגילים של מאמן ה-AI לתרגילים שהמתאמן ביצע. שתי החלטות נעולות: (1) soft-delete לא נספר — reuse של `getDistinctPerformedExerciseIds` כמו שהיא (מחריגה אימונים שנמחקו); (2) cardio/חימום **פטור** מהצמצום, core וכל תרגילי הכוח **כפופים** לו. | נוסף ב-PR #143: נקודת סינון **אחת** ב-`aiTrainerService.buildContext` (`src/domains/workouts/services/aiTrainerService.ts:215-218`) — `availableExercises = exercises.filter(ex => isCardio || performedExerciseIds.has(ex.id))`, כאשר `performedExerciseIds = getDistinctPerformedExerciseIds(userId)` (אותו מקור-אמת של השער מ-PR1). מכיוון שהסינון מצמצם את `context.availableExercises` עצמו, הוא מתפשט אוטומטית ל-payload (`:60`), ל-fallback הלקוח (`strengthExercises ⊂ availableForToday ⊂ availableExercises`, `:295-313`), ול-fallback השרת (`data.availableExercises`, `generateWorkout.ts:610/631/638/647`) — אין סינון שני. cardio מזוהה כמו בשרת: `primaryMuscle==='cardio' || category==='cardio'` (cast ל-string כי `MuscleGroup` לא מונה cardio אך Firestore הוא מקור-האמת). קצה cold-start: בריכה רק-cardio (אפס כוח שבוצע) → לא קורס (רשימות השרירים נגזרות מקטלוג `muscles` ב-Firestore, לא מהבריכה → אין חלוקה-באפס); כשל קריאה → נתיב השגיאה החינני הקיים (`success:false`), לא throw. 7 בדיקות התנהגותיות חדשות (`tests/aiTrainerPoolFilter.spec.ts`, mock לתלויות + assert על ה-payload שנלכד ל-callable, כולל נתיב throw). `/code-review` high — כל ממצאי ה-correctness הופרכו מהקוד. **לא נגעתי**: Cloud Function, `useActiveWorkout.ts`, `getPersonalRecords`. אין כתיבות Firestore. אומת במכשיר לפני merge. |
| 24/06/2026 | פיצ'ר (PR3 מתוך 3, UI בלבד): הודעת הסבר למשתמש שהאימון נבנה מהתרגילים שכבר ביצע. **חוק רצף נעול: #143 (סינון) חייב להתמזג לפני #144 (הודעה) — לעולם לא הפוך**, אחרת ההודעה שקרית (Phase 0 אבחן שעל ענף PR3 לבדו, ללא הסינון של PR2, המאגר הוא הקטלוג המלא וההודעה אינה אומרת אמת). | נוסף ב-PR #144: הודעה קצרה בתוך פופאפ ההסבר הקיים (`AITrainerModal.tsx:491-497`): "בנינו לך את האימון מהתרגילים שכבר ביצעת — אתה מכיר אותם, ויש לך עליהם היסטוריה". כשמספר התרגילים השונים מ-state הפתיחה (PR1, `distinctExerciseCount`) זמין → משולב "מתוך N תרגילים שביצעת עד כה"; אם null → המספר מושמט ללא קריאות נתונים נוספות. RTL, tailwind בלבד, ללא inline styles. **רצף המיזוג נאכף בפועל**: #144 עבר rebase על main אחרי מיזוג #143, כך שההודעה והסינון נמצאים יחד ב-main (`a55104b` לפני `382f6fa`) — ההודעה אומרת אמת בפרודקשן. בדיקת קומפוננטה התנהגותית (`tests/aiTrainerExplanationMessage.spec.tsx`): פתיחה → שער → יצירה → פופאפ → assert שההודעה מרונדרת עם המספר. **לא נגעתי**: לוגיקת יצירה, payload, סינון PR2, `useActiveWorkout.ts`, `getPersonalRecords`. אין כתיבות Firestore. אומת במכשיר לפני merge. |
| 24/06/2026 | פיצ'ר חדש "בורר מקור תרגילים" (תכנית 3-PR שנייה, נפרדת מה-#143/#144 שלמעלה): PR-1 — הזרמת דגל `exerciseSource: 'performed' \| 'all'` מקצה-לקצה עם ברירת מחדל ששומרת התנהגות נוכחית בדיוק, ללא צריכה בלוגיקה עדיין. | נוסף ב-PR #146: `ExerciseSource` type alias + שדה אופציונלי על `AITrainerRequest` (`aiTrainer.types.ts`) ועל `GenerateWorkoutRequest.request` בשרת (`functions/src/ai-trainer/types.ts`) — מסונכרן מהרגע הראשון כי הטיפוס כפול לקוח/שרת. `AITrainerModal` מחזיק state פנימי (`'performed'`) ומוסיף את הדגל ל-object הבקשה ב-`handleGenerate` (תמיד מועבר, לעולם לא undefined). **ללא UI.** הדגל זורם דרך `payload = { request: context.request }` בלי לערוך את `aiTrainerService`. 3 בדיקות התנהגותיות (`tests/aiTrainerSourceFlag.spec.ts`, mock + assert על ה-payload שנלכד). אין שינוי התנהגותי, אין כתיבות Firestore. |
| 25/06/2026 | "בורר מקור תרגילים" PR-2: הדגל `exerciseSource` שולט כעת **בפועל** בבריכת התרגילים — עדיין ללא UI (ברירת מחדל `'performed'`). אכיפה **לקוח-סמכותית** (החלטה נעולה): הלקוח בונה את הבריכה לפי הדגל, השרת צורך את מה שנשלח ואינו מחשב מחדש performed-set. | נוסף ב-PR #147: ב-`aiTrainerService.buildContext` הסינון הפך מותנה — `exerciseSource === 'all'` → כל הספרייה ללא סינון; אחרת (כולל undefined/default) → אותו סינון performed כמו קודם, כולל פטור cardio, רק עטוף בתנאי. `Promise.all` לא נגע. בשרת (`generateWorkout.ts`): `validateRequest` מנרמל ערך חסר/לא-תקין ל-`'performed'` בלי לזרוק; לוג אבחון יחיד (`functions.logger.info`) של המצב + גודל הבריכה, **אחרי ה-awaits**, Cloud Logging בלבד — אפס כתיבות Firestore, אפס חישוב performed-set/שינוי בריכה. ה-`console.log` בלקוח הפך מודע-מצב (תיקון ממצא `/code-review`). 5 בדיקות התנהגותיות חדשות (`aiTrainerPoolFilter` + `aiTrainerSourceFlag`). `functions/node_modules` הותקן ו-`tsc -p functions` ירוק. אומת במכשיר לפני merge. **לא נגעתי**: UI/שער/הודעה, `useActiveWorkout.ts`, `getPersonalRecords`. הערה: במצב `'all'` `getDistinctPerformedExerciseIds` עדיין רץ ב-Promise.all (ערך לא בשימוש) → כשל קריאה עדיין יכשיל יצירה גם ב-'all' (תוצר האילוץ "אל תיגע ב-Promise.all"). |
| 25/06/2026 | תקלה (דווחה ע"י המשתמש, מובייל): במסך מאמן-AI, כשסליידר כמות האימונים < 3 (1 או 2) לא ניתן היה לבחור מבנה אימון (כל הגוף / לפי אזורים) — נכפה Full Body. רצוי שבחירת המבנה תהיה זמינה תמיד. | תוקן ב-PR #148: ב-`AITrainerModal.tsx` הוסר ה-override `effectiveStructure = numWorkouts <= 2 ? 'full_body' : workoutStructure` (הבקשה שולחת כעת את `workoutStructure` של המשתמש לכל מספר אימונים), והוסרה תיבת ה-Full Body הקבועה — בורר המבנה מוצג **תמיד**. בורר עליון/תחתון הוכלל מ-{3,5} למספרים אי-זוגיים (`numWorkouts % 2 === 1`, כלומר 1,3,5) כך שפיצול באימון בודד ניתן לבחירה בפועל; ה-spread של `splitStartWith` משתמש באותו `showSplitStartSelection`. split ב-1-2 אימונים בטוח (רשימות שרירים מקטלוג, לא מהבריכה → אין חלוקה-באפס). בדיקה התנהגותית חדשה `tests/aiTrainerStructureChoice.spec.tsx`. מוגבל ל-`AITrainerModal` (הבורר ייחודי לזרימת המתאמן). אומת במכשיר לפני merge. |
| 26/06/2026 | "בורר מקור תרגילים" PR-3 (השינוי הנראה למשתמש): המתאמן בוחר כעת בעצמו את מקור התרגילים, ושער הכניסה (PR1) **כבר לא חוסם את כל הטופס**. ההחלטה הנעולה: מתחת לסף ה-distinct, במקום מסך חסימה — הטופס מוצג, אפשרות `'performed'` מושבתת עם הסבר "כמה עוד", וברירת המחדל עוברת ל-`'all'`. | נוסף ב-PR #150: ב-`AITrainerModal.tsx` — `exerciseSource` הפך user-selectable (נוסף setter) עם בורר חדש (tailwind בלבד, עקבי עם אופציות DURATION/WARMUP): שני כפתורים "💪 מתוך שביצעת" ו-"🗂️ מכל התרגילים". השער **פורש מחדש**: הוסר חסם הטופס הגורף — הטופס מוצג ברגע שבדיקת ההיסטוריה מתרסקת; מתחת לסף השער רק (א) משבית את אופציית `'performed'` עם שורת הסבר "כמה עוד" (העתק `exercisesRemaining` הישן, הועבר לכאן) ו-(ב) קובע ברירת מחדל. ברירת המחדל מבוססת-סף ונקבעת **פעם אחת** כשהספירה מתרסקת (מתחת → `'all'`, בסף ומעלה → `'performed'`); בחירה ידנית נשמרת כי effect הפתיחה רץ מחדש רק בפתיחה/שינוי משתמש. הערת הפופאפ של #144 ("בנינו מהתרגילים שביצעת") מוצגת רק במצב `'performed'` (ב-`'all'` הייתה שקרית). ב-`aiTrainerService.ts` — מצב `'all'` כבר לא קורא ל-`getDistinctPerformedExerciseIds` (ה-slot ב-`Promise.all` הפך `Promise.resolve(new Set())`), כך שכשל הקריאה הזו כבר **לא** מפיל יצירת `'all'` (סוגר את ההערה שנותרה פתוחה ב-PR #147); `'performed'`/default byte-equivalent (אותה קריאה, אותו מיקום, rejection עדיין מתפשט). בדיקות התנהגותיות (mock + assert): `tests/aiTrainerModalSelector.spec.tsx` (ברירות מחדל מתחת/בסף, אופציה מושבתת, הצגת/הסתרת ההודעה, שמירת בחירה ידנית) + `tests/aiTrainerPoolFilter.spec.ts` (+2: `'all'` מייצר כשהקריאה הייתה נכשלת ואינה נקראת; `'performed'` ללא שינוי). **לא נגעתי**: בורר המבנה/סליידר/`showSplitStartSelection` (#148), Cloud Function, `useActiveWorkout.ts`, `getPersonalRecords`. ללא inline styles חדשים. אומת: tsc נקי, 239 טסטים ירוקים, `/code-review` נקי. אומת במכשיר לפני merge. |
| 16/07/2026 | **תקרית חוזרת — ישבן (המשך ל-14/03/2026):** מאמן ה-AI לא הכניס תרגילי ישבן ביום גוף תחתון. "ישבן" הופיע בפרומפט, אבל אפס תרגילי ישבן שרדו את פילטר השרת: 7 תרגילים תויגו `primaryMuscle=longissimus` (ממופה ל-back) ואחד ריק — שריד של אותו תיוג שבור מ-14/03 שאז תוקן רק בצד הניתוח, לא בנתונים עצמם. בנוסף נמצאו 23 תרגילים עם primaryMuscle ריק, 3 עם `adductor` (יחיד, לא ממופה) ותקלדה אחת. | **הפעם תוקן גם השורש בנתונים וגם השבירות בקוד.** נתונים: 32 מסמכי exercises תוקנו ב-production (גיבוי מלא ב-`scripts/backups/`, סקריפט: `scripts/fix-primary-muscles-2026-07.ts`); עיקרון נעול: primaryMuscle נשאר נכון אנטומית ולא מותאם ל-category שגוי. קוד: `SUB_MUSCLE_TO_PARENT` חולץ ל-`functions/src/ai-trainer/muscleMapping.ts` (משותף), נוספו `adductor`/`abductor`, ופילטר ה-GPT נופל ל-category עם `functions.logger.warn` (לא שקט) כש-primaryMuscle לא נפתר. ולידציה: `scripts/validate-exercise-muscles.ts` (exit 1 על primaryMuscle לא ממופה) — להריץ אחרי כל שינוי בתרגילים/שרירים. |
| 02-03/08/2026 | תקרית (zehava@assa-adam.com, 02/08 08:53): מחיקת התרגיל האחרון באימון AI פעיל רוקנה את מערך התרגילים — המסך קרס ל"אין אימון פעיל" וה-autosave דרס את המסמך המקורי כאימון `in_progress` ריק שנעלם מההיסטוריה ושוחזר כמסך ריק בכל טעינה. אבחון בפרודקשן מצא 2 מסמכים ריקים (zehava + משתמש נוסף מ-02/2026). | תוקן ב-PR #157 בשלוש שכבות: (1) `deleteExercise`/`confirmDeleteExercise` חוסמים מחיקת תרגיל אחרון + רצפה בתוך ה-reducer נגד double-tap; (2) `autoSaveWorkout` מסרב לרוקן מסמך קיים (`hasNoExercises` משותף, guard לפני בניית payload); (3) `getInProgressWorkout` מדלג על מסמך ריק ועושה soft-delete (לא cancelled — כרטיס cancelled ריק ניתן להמשכה ומשחזר את הבאג). סקריפטים: `findEmptyInProgressWorkouts` (read-only) / `fixEmptyInProgressWorkouts` (dry-run + גיבוי). 9 בדיקות התנהגותיות RED-לפני-תיקון (`tests/emptyWorkoutGuard.spec.ts`). |
| 03/08/2026 | פיצ'ר (PR #158): עריכת שם + קיבוע אימונים במסך האימונים של המתאמן. סדר נעול: מתוכננים (ללא שינוי) → נעוצים (עוקפים את חלון השבועיים, לפי ביצוע אחרון) → שבועיים אחרונים. הוחלט במפורש: מתאמן בלבד, מודול המאמן לא נגע. | שדה `pinned?: boolean` חדש ב-`workoutHistory` + `updateWorkoutHistory` הורחב ל-`name`/`pinned` (בדיקות `!== undefined`). `getUserWorkoutHistory` ממזג שאילתת pinned **ב-opt-in בלבד** (`includePinnedBeyondLimit` — ממצא code-review: מיזוג גורף היה מזהם את הקשר ה-AI ותצוגות המאמן ב-7 call-sites). לוגיקת המיון חולצה ל-`partitionWorkouts.ts` (טהורה ונבדקת). UI: נעץ + עיפרון ב-`WorkoutCard` (44px), דיאלוג שינוי שם בסגנון דיאלוג המחיקה, חסימת impersonation, עדכון אופטימי עם revert. 12 בדיקות התנהגותיות (`tests/workoutPinRename.spec.ts`). ללא rules/index חדשים. |
| 17/09/2026 | הוקם צינור הוספת תרגילים מקצה-לקצה בטרמינל: PR #175 (חילוץ exerciseSchema+toExercisePayload למודול משותף), #176 (writeExercise.ts — המסלול היחיד לכתיבת תרגיל מסקריפט: סכמה+קטלוגים חיים+כפילויות+HEAD תמונה, dry-run כברירת מחדל), #177 (סקיל /exercise-add עם שני גייטים + record-builder + upload-image.sh + קובצי סגנון), #178 (--dedupe-only; ביניים PNG, דחיסת WebP אחת), #179 (תרגילים דומים בגייט 1 — matching/similarExercises.ts). הוקמו: Standing Dumbbell Shoulder Press, Lat Pulldown (גייט 2 פסל סבב ראשון — צילום המכשיר האמיתי תיקן את ה-poseSpec). Machine Lens B.1 (PR #180) נפרס: identifyMachine (gpt-4.1 vision) + התאמה לקטלוג; שיעור פריסה: callable v2 חדשה דורשת gcloud add-invoker-policy-binding (אחרת 401). |
| 18/09/2026 | B.2 — הוספת תרגיל מממשק האדמין (PR #182) + PR #181 (הערות ציוד): הזיהוי החי במכון בוטל (אין קליטה); DraftPanel בטופס האדמין — טיוטת AI משם או מצילום שמור (generateExerciseDraft 30/יום, generateExerciseImage — sunburst+sharp ל-Storage 20/יום, markDraftSaved), הכול fail-closed; מקורות אמת יחידים ב-assets/ מוטמעים ב-functions ב-prebuild עם טסט hash; exercise-record-rules.md הועבר מהסקיל; הערות ציוד מוגרו ל-settings/gymEquipmentNotes; rules+69 טסטים. באג שנתפס באמולטור: admin.firestore.FieldValue דרך importStar — undefined ב-Node מקומי (ב-GCF עובד); תוקן בפונקציות החדשות בייבוא ישיר firebase-admin/firestore. אומת: שני מסלולי אמולטור עם OpenAI אמיתי + smoke בפרודקשן. |
| 16/07/2026 | תקלה (דווחה ע"י המשתמש, מובייל): כרטיס תרגיל הציג "מומלץ להגדיל משקל" (דגל סטגנציה, מחושב בלקוח) ולצדו המלצת AI של 9kg — מתחת לשיא ולאימון האחרון (10kg). שורש: הפרומפט התיר רצפה `X*0.9` (+שתי שורות סותרות ±5% מול 0.9–1.05), ואף שכבה לא יישבה בין הדגל למספר — הדגל חי רק בלקוח (`exerciseRecommendations/{userId}`), המספר נולד בשרת. אומת: אין שום מסלול deload בקוד. | תוקן בשרת (הנקודה היחידה עם כל שלושת האותות לפני כתיבת ה-bundle): `generateWorkout` קורא את הדגלים ב-Admin SDK, מזריק `stagnant:true` לפרומפט, ומחיל רצפה במודול טהור `functions/src/ai-trainer/stagnationFloor.ts` — דגל דלוק + המלצה ≤ אחרון → דריסה ל-`max(roundToHalf(last*1.05), last+0.5)` (גדול **ממש**). בלי דגל — אין רצפה (לא לחסום deload עתידי בשקט). כל דריסה/דגל-חסר → `functions.logger.warn` (No silent AI filtering). פרומפט יושב ל-0.95–1.05. 6 בדיקות התנהגותיות RED-לפני-תיקון. מגבלה: המלצות ב-localStorage מאימונים קיימים נשארות שגויות עד אימון חדש. סטטוס: fix candidate, ממתין לאימות מכשיר אחרי deploy. |

---

```
══════════════════════════════════════════════════════════════════════════════
עדכון אחרון: 18/09/2026 | (1) צינור /exercise-add מלא בטרמינל (PRs #175-#179); (2) Machine Lens B.1 נפרס והזיהוי החי בוטל; (3) B.2 — הוספת תרגיל מהאדמין (שם/צילום, טיוטת AI, תמונה, שמירה מגודרת) בפרודקשן (PRs #181-#182).
══════════════════════════════════════════════════════════════════════════════
```
