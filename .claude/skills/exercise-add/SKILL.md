...
מתחת לכל רשומה: **"תרגילים דומים שכבר קיימים"** — הפלט של `--dedupe-only` (עד 3, עם score ו-reasons; אפשר ללכוד כ-JSON עם `--similar-json <path>`).
**כלל:** אם יש דומה עם **אותה category ואותו equipment** — ברירת המחדל היא **לעצור את התרגיל ולשאול**, לא להמשיך.
