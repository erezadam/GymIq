...
הפרויקט משתמש ב-**Vitest בלבד** לטסטים אוטומטיים. אין E2E browser framework.

```bash
npm run build   # TypeScript + Vite — חייב לעבור
npm test        # Vitest unit + regression — חייב לעבור
```

לאימות זרימה ידנית במכשיר אחרי deploy — בדיקה ידנית על production. אם נדרש
E2E browser test לסיבה ספציפית — להריץ מחוץ לפרויקט (לא תשתית פנימית).
