...
## 🎭 Playwright MCP — בדיקה ויזואלית משלימה

> Playwright MCP מאפשר ל-Claude Code לשלוט בדפדפן לבדיקות ויזואליות.
> **זה כלי משלים — לא תחליף לטסטים האוטומטיים!**

### מתי להשתמש:
- אימות ויזואלי של UI אחרי שינוי עיצוב
- בדיקת RTL/מובייל ויזואלית
- דיבוג אינטראקטיבי של בעיות תצוגה

### מתי לא:
- לא במקום `npm run test:e2e` — טסטים אוטומטיים תמיד נדרשים לפי רמות הבדיקה
- לא כ"הוכחה" שפיצ'ר עובד

### ⚠️ חוק ברזל: אישור מפורש ל-Playwright
**כל הרצת `npx playwright test` (spec בודד או suite מלא) דורשת אישור מפורש מהמשתמש לפני ההרצה.**
הסוכן חייב לשאול: "האם להריץ טסטי Playwright?" ולחכות לאישור.
לעולם לא להריץ Playwright באופן אוטונומי.
